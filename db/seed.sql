TRUNCATE users, category_group, category, health_event RESTART IDENTITY CASCADE;

-- Ensure category_group data is populated first if not handled by Node
...
('Clinical Care & Logistics')
ON CONFLICT DO NOTHING;

-- Seed Categories with standardized group links
INSERT INTO category (category_name, category_type, definition, icon, color, description, category_group_id) VALUES
('Headache', 'health', 'system', 'thermometer', '#FF6B6B', 'Track headache severity, location and triggers', 
  (SELECT id FROM category_group WHERE name = 'Neurological & Chronic Pain')),

('Cough', 'health', 'system', 'wind', '#74B9FF', 'Track cough type, severity and duration', 
  (SELECT id FROM category_group WHERE name = 'Respiratory & Breathing')),

('Fever', 'health', 'system', 'thermometer', '#E17055', 'Track fever temperature and duration', 
  (SELECT id FROM category_group WHERE name = 'General Sickness & Infection')),
...
('Nausea', 'health', 'system', 'alert-circle', '#FF6B81', 'Track nausea severity and triggers', 
  (SELECT id FROM category_group WHERE name = 'Gastrointestinal & Digestion')),
...
('Chest Discomfort', 'health', 'system', 'heart', '#FF6B6B', 'Track chest discomfort type and severity', 
  (SELECT id FROM category_group WHERE name = 'Cardiovascular & Vital Signs')),
...
('Rash', 'health', 'system', 'shield', '#81ECEC', 'Track rash location, appearance and spread', 
  (SELECT id FROM category_group WHERE name = 'Dermatological & Skin')),
...
('Anxiety', 'health', 'system', 'zap', '#FDCB6E', 'Track anxiety levels and triggers', 
  (SELECT id FROM category_group WHERE name = 'Mental Health & Behavioral Wellness')),
...
('Medication', 'health', 'system', 'pill', '#54A0FF', 'Track medication and side effects', 
  (SELECT id FROM category_group WHERE name = 'Clinical Care & Logistics')),
...
('Lab Tests', 'health', 'health', 'system', 'flask', '#00B894', 'Track lab test results and bloodwork', 
  (SELECT id FROM category_group WHERE name = 'Clinical Care & Logistics')),
...
('Diet', 'health', 'system', 'coffee', '#00CEC9', 'Track meals, nutrition, and eating habits', 
  (SELECT id FROM category_group WHERE name = 'Lifestyle, Diet & Activity')),
