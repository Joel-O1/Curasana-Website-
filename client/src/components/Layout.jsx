...
import {
    ...
    Bell, Plus, FileOutput, LogOut, Heart, Logs,
} from "lucide-react";
import { useAuth } from "../contexts/AuthContext";

const NAV = [
    { to: "/dashboard", label: "Dashboard", icon: LayoutDashboard },
    { to: "/log-events", label: "Log Events", icon: Plus },
    { to: "/health-events", label: "Health Events", icon: Activity },
...
const TITLES = {
    ...
    "/log-events": { title: "Log Events", sub: () => "Enter event to track physical health" },
    "/health-events": { title: "Health Events", sub: () => "Track your physical signals over time" },
    "/prescriptions": { title: "Prescriptions", sub: () => "Daily schedule and full prescription log" },
    ...
    "/settings/personal-info": { title: "Personal Information", sub: () => "Date of birth, height, weight, and address" },
    "/settings/password": { title: "Change Password", sub: () => "Update your account password" },
    "/settings/legal": { title: "Legal & Privacy", sub: () => "Terms of service and data practices" },
};
        ...
        <div className="min-h-screen flex" style={{ background: "var(--bg)" }}>
            <aside className="w-64 shrink-0 sticky top-0 h-screen overflow-y-auto" style={{ background: "var(--surface)", borderRight: "1px solid var(--border)" }} data-testid="sidebar">
                ...
                <div className="mt-2 mx-3 p-3 rounded-xl" style={{ background: "var(--gray-soft)" }}>
                    ...
                    <button type="button" onClick={signOut} className="cu-btn cu-btn-ghost mt-3" style={{ width: "100%", justifyContent: "center", fontSize: 12 }} data-testid="sidebar-logout">
            ...
            <main className="flex-1 min-w-0">
                <div className="px-8 py-5 flex items-center justify-between sticky top-0 z-30" style={{ background: "var(--bg)", borderBottom: "1px solid var(--border-soft)" }} data-testid="topbar">
                    ...
                    <div className="flex items-center gap-2">
                        <Link to="/log-events" className="cu-btn cu-btn-ghost" data-testid="topbar-log-symptom">
                            <Plus size={14} /> Log Health Event
                        </Link>
